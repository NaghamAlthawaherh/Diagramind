...
function calculateFinalEstimationHandler() {
  const adjustmentSelects = document.querySelectorAll('.gsc-select');
  let totalGSC = 0;
  adjustmentSelects.forEach(select => totalGSC += parseInt(select.value, 10));
  ...
  // Cost Estimation
  const minBudget = effortManMonths * salary * 0.9;
  const maxBudget = effortManMonths * salary * 1.1;

  // Timeline
  const durationWeeks = Math.ceil(effortManMonths * 4);
