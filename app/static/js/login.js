...
const passwordStrengthBar = document.querySelector('.password-strength progress');
...
function calculatePasswordStrength(password) {
    ...
    if (!commonPasswords.includes(password.toLowerCase())) strength += 15;
...
async function handleLogin(e) {
    ...
    try {
        ...
        if (response.ok) {
            // Successful login
            window.location.href = data.redirect || '/index1';
        } else {
            // Show error message
...
langSwitcher.addEventListener('change', (e) => {
    ...
    // This would typically involve fetching translation files or reloading the page with new lang param
});
...
document.querySelectorAll('.social-btn').forEach(btn => {
    btn.addEventListener('click', (e) => {
        ...
        showToast(`Redirecting to ${provider} login...`, false);
...
// Accessibility: Handle form submission on Enter key in password field
